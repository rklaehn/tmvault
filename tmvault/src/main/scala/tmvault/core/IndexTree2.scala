...
sealed abstract class IndexTree2 {
   ...
   */
  def split: (IndexTree2, IndexTree2)
...
object IndexTree2 {
  ...
  def fromLong(value:Long) : IndexTree2 = mkLeaf(Array[Long](value))

  def fromLongs(values:Array[Long]) : IndexTree2 = {
    require(values.length > 0)
    require(values.isIncreasing())
    def mkNode(from:Int, until:Int) : IndexTree2 = {
  ...
  def lines(tree:IndexTree2, indent:String = "  ") : Traversable[String] = new Traversable[String] {
    override def foreach[U](f: (String) => U): Unit = show(tree, indent)(x => f(x))
  }

  def show(tree:IndexTree2, indent:String = "  ")(f:String => Unit = println) : Unit = {
    def show0(tree:IndexTree2, prefix:String) : Unit = tree match {
  ...
  def merge(a: IndexTree2, b: IndexTree2): IndexTree2 = {
  ...
  final case class Leaf(min: Long, level: Int, data: Array[Long]) extends IndexTree2 {
  ...
  final case class Branch(min: Long, level: Int, left: IndexTree2, right: IndexTree2) extends IndexTree2 {
